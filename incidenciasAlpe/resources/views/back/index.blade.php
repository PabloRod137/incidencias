...
@section('content')
    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
        <x-back.stat-card label="Incidencias" :value="$stats['incidencias']" :route="route('incidencias.index')" icon="🎫" color="blue" />
        @if(Auth::user()->role === 'admin')
            <x-back.stat-card label="Usuarios" :value="$stats['usuarios']" :route="route('usuarios.index')" icon="👥" color="green" />
            <x-back.stat-card label="Aulas" :value="$stats['aulas']" :route="route('aulas.index')" icon="🏫" color="orange" />
            <x-back.stat-card label="Categorías" :value="$stats['categorias']" :route="route('categorias.index')" icon="📁" color="purple" />
        @endif
    </div>
